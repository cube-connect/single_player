...
ObjectManager::ObjectManager(class MyScene& owner)
    : m_Scene(owner)
    , m_NextObjectID(0)
    , m_ToInitialize(0)
...
MyObject* ObjectManager::CreateObject(std::string name) {
    m_Objects.push_back(std::make_unique<MyObject>(*this, m_NextObjectID, name));
...
void ObjectManager::DestroyObject(std::uint8_t id) {
    auto object = std::find_if(m_Objects.begin(),
                               m_Objects.end(),
                               [=](std::unique_ptr<MyObject>& obj) { return obj->ID() == id; });
