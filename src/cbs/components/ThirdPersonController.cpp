...
ThirdPersonController::ThirdPersonController(class MyObject *target, glm::vec3 front, glm::vec3 right, float radius,
                                             float mouse_sensitivity, bool invert_yaw_movement)
    : m_Target(target)
...
void ThirdPersonController::Update() {
    ...
    if (g_Input.KeyHold(GLFW_MOUSE_BUTTON_2)) {
        ...
        m_YRotation += (m_InvertYawMovement ? 1 : -1) * glm::radians(g_Input.MouseOffset().x * m_MouseSensitivity);
    } else {
