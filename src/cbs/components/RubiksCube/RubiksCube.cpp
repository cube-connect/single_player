...
void RubiksCube::MakeConnectors(MessageManager& message_manager) {
    message_manager.Make(this, m_CurrentMovesToBePerformedOut);
}
...
void RubiksCube::Update() {
    ...
    if (!m_CurrentMovesToBePerformed.empty()) {
        if (!m_CurrentMovesToBePerformed.front()->Finished()) {
            m_CurrentMovesToBePerformed.front()->Progress(g_Time.FixedDeltaTime());
        } else {
            m_CurrentMovesToBePerformed.pop_front();
            UpdateTextRenderer();
        }
    }
}
...
void RubiksCube::RotateFace(EFace face, ERotation rotation) {
    ...
    m_CurrentMovesToBePerformed.emplace_back(std::make_unique<FaceRotation>(*this, face_to_rotate, rotation, 90.0f, FACE_ROTATION_SPEED));
    UpdateTextRenderer();
}

void RubiksCube::RotateCube(EDirection direction, ERotation rotation) {
    m_CurrentMovesToBePerformed.emplace_back(std::make_unique<CubeRotation>(*this, &Object().Root(), direction, rotation, CUBE_ROTATION_SPEED));
    UpdateTextRenderer();
}

void RubiksCube::Randomize(unsigned int moves) {
    if (m_CurrentMovesToBePerformed.size() > 0) {
        // Finish current task
        m_CurrentMovesToBePerformed.front()->Progress(1.0f);
        m_CurrentMovesToBePerformed.clear();
    }
...
void RubiksCube::UpdateTextRenderer() {
    std::string message;
    message.reserve(m_CurrentMovesToBePerformed.size() * 2);
    for (auto it = m_CurrentMovesToBePerformed.begin(); it != m_CurrentMovesToBePerformed.end(); it++) {
        message += ' ' + (*it)->TaskName();
    }
    m_CurrentMovesToBePerformedOut.Send(message);
}
