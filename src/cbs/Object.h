...
public:
    MyObject(ObjectManager& scene, std::uint8_t id, std::string name = "object");
     ...
     * At the time of calling constructor new Component will not have access to it's MyObject-owner.
     * Futhermore component receives it's own ID greater than 0. All MyObject
     ...
     * lifetime is bounded to MyObject lifetime.
    ...
    void RemoveComponents() {
        ...
        for (auto& comp : m_Components) {
            if (dynamic_cast<T*>(comp.get()) != nullptr) {
                MarkToDestroy(
                        reinterpret_cast<__gnu_cxx::__normal_iterator<std::unique_ptr<Component> *, std::vector<std::unique_ptr<Component>>> &&>(comp));
            }
        }
    }
    ...
    void Connect(MessageOut<M>& sender, MessageIn<M, O, F>& receiver) {
        assert(sender.Owner()->MyObject().ID() == m_ID && receiver.Owner()->MyObject().ID() == m_ID);
    ...
    void Connect(TriggerOut& sender, TriggerIn<O, F>& receiver) {
        assert(sender.Owner()->Object().ID() == m_ID && receiver.Owner()->MyObject().ID() == m_ID);
        m_MessageManager.Connect(sender, receiver);
    }
...
private:
    ...
    // All components owned by MyObject
    Transform m_Root;
